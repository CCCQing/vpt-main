...
# 鍩虹杩愯閰嶇疆
# -------------------------------
_C.DBG = False                         # 鍏ㄥ眬璋冭瘯寮€鍏?_C.OUTPUT_DIR = "./output"             # 璁粌杈撳嚭鐩綍
_C.RUN_N_TIMES = 5                     # 閲嶅杩愯娆℃暟锛堢敤浜庡娆″疄楠屽彇骞冲潎锛?
# CUDNN benchmark锛?# 褰撹緭鍏ュ昂瀵稿浐瀹氭椂鍙兘鍔犻€燂紱褰撹緭鍏ュ昂瀵稿彲鍙樻椂锛屽彲鑳藉鍔犳樉瀛樺崰鐢ㄤ笖鏀剁泭鏈夐檺
_C.CUDNN_BENCHMARK = False

# 鍒嗗竷寮?/ 闅忔満鎬?_C.NUM_GPUS = 1                        # 浣跨敤 GPU 鏁伴噺锛堣缁?娴嬭瘯閫氱敤锛?_C.NUM_SHARDS = 1                      # 鍒嗙墖鏁帮紙澶氭満璁粌鏃朵娇鐢級
_C.SEED = None                         # 闅忔満绉嶅瓙锛涙敞鎰?GPU 绠楀瓙浠嶅彲鑳藉瓨鍦ㄩ潪纭畾鎬?
# 鍏煎鏃ч厤缃殑椤跺眰寮€鍏?_C.GZSL = False                        # 鏃х増鍏煎锛氭槸鍚﹂噰鐢?GZSL 妯″紡锛堝疄闄呬互 SOLVER.EVAL_MODE 涓哄噯锛?_C.USE_TRAINVAL = False                # 鏃х増鍏煎锛氭槸鍚﹀湪璁粌涓娇鐢?trainval锛堝疄闄呭缓璁湅 DATA.XLSA.TRAIN_USE_TRAINVAL锛?
...
_C.MODEL = CfgNode()
# 杩佺Щ瀛︿範绫诲瀷
_C.MODEL.TRANSFER_TYPE = "prompt"  # one of linear, end2end, prompt, adapter, side, partial-1, tinytl-bias 鍙€?_C.MODEL.WEIGHT_PATH = ""              # 鑻ヤ粠 checkpoint 鎭㈠锛屽垯鎸囧畾璺緞
...
#    (4) "cls-nofirstl": 浣跨敤鏈€鍚?12 涓?cls 宓屽叆锛堜笉鍖呭惈杈撳叆鍒扮涓€灞傜殑閮ㄥ垎锛?_C.MODEL.PROMPT.INITIATION = "random"
_C.MODEL.PROMPT.CLSEMB_FOLDER = ""     # cls embedding 鎵€鍦ㄧ洰褰曪紙鑻ヤ娇鐢ㄧ壒寰佸垵濮嬪寲锛?_C.MODEL.PROMPT.CLSEMB_PATH = ""       # cls embedding 鏂囦欢璺緞锛堣嫢鐩存帴鎸囧畾鏂囦欢锛?
# Deep prompt 璁剧疆
_C.MODEL.PROMPT.DEEP = True            # 鏄惁鍚敤 deep prompt锛堜粎 prepend 妯″紡鏈夋晥锛?_C.MODEL.PROMPT.NUM_DEEP_LAYERS = None # 鑻ヨ涓?int锛屽垯鍙湪閮ㄥ垎灞備娇鐢?deep prompt
...
# Prompt 璁粌鐩稿叧
_C.MODEL.PROMPT.DROPOUT = 0.0       # 瀵?prompt 宓屽叆鍋氶殢鏈虹疆闆讹紝鐢ㄤ簬姝ｅ垯鍖栵紝缂撹В鎻愮ず杩囨嫙鍚?_C.MODEL.PROMPT.DETACH_PROMPT_GRAD = False     # 鏄惁鍒囨柇 prompt 鍙傛暟姊害
_C.MODEL.PROMPT.DISTRIBUTION_ONLY = True
# True: 瀹屽叏渚濊禆 runtime provider / 鍒嗗竷寮忕敓鎴?prompt锛屼笉鍒涘缓甯歌鍙缁?prompt 鍙傛暟
...
# -----------------------------------------------------------------------------
# Semantic concept module锛堟棫鐗堝叡浜蹇?璇箟-瑙嗚瀵归綈妯″潡锛?# 璇存槑锛?#   杩欐槸浣犳棫妗嗘灦涓殑鈥淪haredConceptAligner / 鍏变韩姒傚康妲解€濈浉鍏宠缃€?#   濡傛灉鍚庣画瀹屽叏鍒囧埌 AGR / affinity role migration锛岃繖鍧楀彲浠ラ€愭寮卞寲銆?# -----------------------------------------------------------------------------
...
_C.MODEL.PROMPT.SEMANTIC_CONCEPT.NUM_HEADS = 4        # 澶氬ご娉ㄦ剰鍔涘ご鏁?_C.MODEL.PROMPT.SEMANTIC_CONCEPT.DROPOUT = 0.0
...
# -----------------------------------------------------------------------------
# Prompt distributor锛圥re-ViT prompt distribution generator锛?# 璇存槑锛?#   褰?PROMPT.DISTRIBUTION_ONLY=True 鏃讹紝闇€瑕?runtime provider 鐢熸垚绗?0 灞?prompt銆?#   鏈ā鍧楀嵆鐢ㄤ簬鏋勫缓璇?provider銆?# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Consistency loss锛圓ENet 椋庢牸鐨勪竴鑷存€ф€濊矾锛屼絾涓嶇瓑浠蜂簬 AENet 缁撴瀯锛?# 璇存槑锛?#   寤鸿鐢ㄤ簬绾︽潫鈥滄渶缁堣涔夊閲?/ 娣卞眰璇箟鍧囧€煎亸绉烩€濈殑鏂瑰悜涓€鑷存€с€?# -----------------------------------------------------------------------------
...
_C.MODEL.PROMPT_FUSION.LAYER_IDS = [0, 6, 11]  # 鍦ㄥ摢浜?Transformer 灞傛墽琛屾彁绀烘洿鏂?_C.MODEL.PROMPT_FUSION.KL_WEIGHT = 1e-4  # KL 姝ｅ垯鏉冮噸锛堣缁冩椂鍙锛?_C.MODEL.PROMPT_FUSION.DROPOUT = 0.0  # 閲囨牱鍚庣殑鎻愮ず鍚戦噺 dropout
...
_C.MODEL.AFFINITY.PROMPT_LENGTH = 0        # prompt 闀垮害锛堢己鐪佹椂鐢?NUM_TOKENS 濉厖锛?_C.MODEL.AFFINITY.RETURN_CROSS = False     # 鏄惁杩斿洖璺ㄦā鎬佷翰鍜?_C.MODEL.AFFINITY.NORMALIZE = True         # 浜插拰鐭╅樀鏄惁褰掍竴鍖?_C.MODEL.AFFINITY.DETACH = True            # 璁＄畻浜插拰鏃舵槸鍚﹀垎绂绘搴?_C.MODEL.AFFINITY.VIS = False              # 鏄惁鍚屾椂杩斿洖娉ㄦ剰鍔涙潈閲嶏紙vis 妯″紡锛?
# ----------------------------------------------------------------------
# adapter options
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
_C.SOLVER = CfgNode()
# 涓绘崯澶辩被鍨?_C.SOLVER.LOSS = "softmax_prompt_align"
...
# 鏂颁富绾跨浉鍏虫崯澶憋紙褰撳墠榛樿鍧囧叧闂紝鐢卞疄楠岄€愭鎵撳紑锛?_C.SOLVER.LOSS_ROLE_EARLY_WEIGHT = 0.0     # 娴呭眰瑙掕壊杩佺Щ鎹熷け鏉冮噸
...
# 浼樺寲鍣?_C.SOLVER.OPTIMIZER = "sgd"  # or "adamw"
...
_C.SOLVER.BASE_LR = 0.25     # 瀛︿範鐜?_C.SOLVER.BIAS_MULTIPLIER = 1.               # prompt / bias 鐨?lr 鍊嶇巼
...
_C.SOLVER.GZSL = False  # 鏃х増鍏煎寮€鍏筹紝寤鸿瀹為檯浠?EVAL_MODE 涓哄噯
...
_C.DATA.XLSA.TEST_INCLUDE_SEEN = False
...
if not hasattr(_C, "OUTPUT_DIR"):
    _C.OUTPUT_DIR = "./output"
if not hasattr(_C, "NUM_GPUS"):
    _C.NUM_GPUS = 1
if not hasattr(_C, "NUM_SHARDS"):
    _C.NUM_SHARDS = 1
if not hasattr(_C, "GZSL"):
    _C.GZSL = False
if not hasattr(_C, "USE_TRAINVAL"):
    _C.USE_TRAINVAL = False
if not hasattr(_C, "MODE"):
    _C.MODE = "xlsa"
if not hasattr(_C, "SKIP_DUMMY"):
    _C.SKIP_DUMMY = False
if not hasattr(_C, "SPLIT"):
    _C.SPLIT = "train"

if not hasattr(_C.MODEL, "WEIGHT_PATH"):
    _C.MODEL.WEIGHT_PATH = ""
if not hasattr(_C.MODEL.LINEAR, "DROPOUT"):
    _C.MODEL.LINEAR.DROPOUT = 0.1
if not hasattr(_C.MODEL.PROMPT, "INITIATION"):
    _C.MODEL.PROMPT.INITIATION = "random"
if not hasattr(_C.MODEL.PROMPT, "CLSEMB_PATH"):
    _C.MODEL.PROMPT.CLSEMB_PATH = ""
if not hasattr(_C.MODEL.PROMPT, "NUM_DEEP_LAYERS"):
    _C.MODEL.PROMPT.NUM_DEEP_LAYERS = None
if not hasattr(_C.MODEL.PROMPT, "DETACH_PROMPT_GRAD"):
    _C.MODEL.PROMPT.DETACH_PROMPT_GRAD = False

_sc = _ensure_node(_C.MODEL.PROMPT, "SEMANTIC_CONCEPT")
if not hasattr(_sc, "ENABLE"):
    _sc.ENABLE = False
if not hasattr(_sc, "NUM_SLOTS"):
    _sc.NUM_SLOTS = 4
if not hasattr(_sc, "NUM_HEADS"):
    _sc.NUM_HEADS = 4
if not hasattr(_sc, "DROPOUT"):
    _sc.DROPOUT = 0.0
if not hasattr(_sc, "LAMBDA_INIT"):
    _sc.LAMBDA_INIT = 1.0
if not hasattr(_sc, "USE_LAYER_NORM"):
    _sc.USE_LAYER_NORM = True
if not hasattr(_sc, "PROJ_NORM"):
    _sc.PROJ_NORM = True

_diag = _ensure_node(_C.SOLVER, "DIAG")
if not hasattr(_diag, "SHUFFLE_RAW_TARGETS"):
    _diag.SHUFFLE_RAW_TARGETS = False
if not hasattr(_diag, "SHUFFLE_PROTOTYPES"):
    _diag.SHUFFLE_PROTOTYPES = False
if not hasattr(_diag, "STRICT_CHECKS"):
    _diag.STRICT_CHECKS = False
if not hasattr(_diag, "PRINT_LOSS_WIRING"):
    _diag.PRINT_LOSS_WIRING = False

_monitor = _ensure_node(_C.SOLVER, "MONITOR")
if not hasattr(_monitor, "ENABLE"):
...
if not hasattr(_C.SOLVER, "LOSS_ALPHA"):
    _C.SOLVER.LOSS_ALPHA = 0.01
if not hasattr(_C.SOLVER, "LOSS_MARGIN"):
    _C.SOLVER.LOSS_MARGIN = 0.05
if not hasattr(_C.SOLVER, "LOSS_CM_WEIGHT"):
    _C.SOLVER.LOSS_CM_WEIGHT = 0.05
if not hasattr(_C.SOLVER, "LOSS_SEM_DIST_WEIGHT"):
    _C.SOLVER.LOSS_SEM_DIST_WEIGHT = 0.0
if not hasattr(_C.SOLVER, "LOSS_SEM_ROUTE_WEIGHT"):
    _C.SOLVER.LOSS_SEM_ROUTE_WEIGHT = 0.0
if not hasattr(_C.SOLVER, "LOSS_HN_MARGIN_ENABLE"):
    _C.SOLVER.LOSS_HN_MARGIN_ENABLE = False
if not hasattr(_C.SOLVER, "LOSS_HN_MARGIN_WEIGHT"):
    _C.SOLVER.LOSS_HN_MARGIN_WEIGHT = 0.05
if not hasattr(_C.SOLVER, "LOSS_HN_MARGIN_VALUE"):
    _C.SOLVER.LOSS_HN_MARGIN_VALUE = 0.1
if not hasattr(_C.SOLVER, "LOSS_HN_MARGIN_START_EPOCH"):
    _C.SOLVER.LOSS_HN_MARGIN_START_EPOCH = 0
if not hasattr(_C.SOLVER, "LOSS_HN_DETACH_NEG"):
    _C.SOLVER.LOSS_HN_DETACH_NEG = True
if not hasattr(_C.SOLVER, "LOSS_ROLE_EARLY_WEIGHT"):
    _C.SOLVER.LOSS_ROLE_EARLY_WEIGHT = 0.0
if not hasattr(_C.SOLVER, "LOSS_ROLE_LATE_WEIGHT"):
    _C.SOLVER.LOSS_ROLE_LATE_WEIGHT = 0.0
if not hasattr(_C.SOLVER, "LOSS_AGR_RES_WEIGHT"):
    _C.SOLVER.LOSS_AGR_RES_WEIGHT = 0.0
if not hasattr(_C.SOLVER, "LOSS_AVS_ENT_WEIGHT"):
...
if not hasattr(_C.SOLVER, "BASE_LR"):
    _C.SOLVER.BASE_LR = 0.25
if not hasattr(_C.SOLVER, "BIAS_MULTIPLIER"):
    _C.SOLVER.BIAS_MULTIPLIER = 1.0
if not hasattr(_C.SOLVER, "EVAL_MODE"):
    _C.SOLVER.EVAL_MODE = "zsl"

if not hasattr(_C.DATA, "NAME"):
    _C.DATA.NAME = "CUB_200_2011"

if not hasattr(_C.MODEL, "SEMANTIC_SCORE_MODE"):
    _C.MODEL.SEMANTIC_SCORE_MODE = "global_refined"
if not hasattr(_C.MODEL, "SEMANTIC_SCORE_TOPK"):
    _C.MODEL.SEMANTIC_SCORE_TOPK = 5
if not hasattr(_C.MODEL, "SEMANTIC_SCORE_ALPHA"):
    _C.MODEL.SEMANTIC_SCORE_ALPHA = 1.0
if not hasattr(_C.MODEL, "SEMANTIC_SCORE_TRAIN_INCLUDE_GT"):
    _C.MODEL.SEMANTIC_SCORE_TRAIN_INCLUDE_GT = True
if not hasattr(_C.MODEL, "SEMANTIC_SCORE_EVAL_OVERRIDE"):
    _C.MODEL.SEMANTIC_SCORE_EVAL_OVERRIDE = ""

_role = _ensure_node(_C.MODEL, "ROLE_MIGRATION")
if not hasattr(_role, "ENABLE"):
    _role.ENABLE = False
if not hasattr(_role, "EARLY_END"):
    _role.EARLY_END = 3
if not hasattr(_role, "LATE_START"):
    _role.LATE_START = 9

_agr = _ensure_node(_C.MODEL, "AGR")
if not hasattr(_agr, "ENABLE"):
    _agr.ENABLE = False
if not hasattr(_agr, "TOPK"):
    _agr.TOPK = 5
if not hasattr(_agr, "ALPHA"):
    _agr.ALPHA = 0.2
if not hasattr(_agr, "FUSE_ALPHA"):
    _agr.FUSE_ALPHA = 0.5
if not hasattr(_agr, "TRAIN_INCLUDE_GT"):
    _agr.TRAIN_INCLUDE_GT = True

_cons = _ensure_node(_C.MODEL, "CONSISTENCY")
if not hasattr(_cons, "ENABLE"):
    _cons.ENABLE = False
if not hasattr(_cons, "PROJ"):
    _cons.PROJ = "linear"
if not hasattr(_cons, "DIST"):
    _cons.DIST = "cosine"

_sem_branch = _ensure_node(_C.MODEL, "SEMANTIC_BRANCH")
if not hasattr(_sem_branch, "ENABLE"):
...
if not hasattr(_sem_branch, "END_LAYER"):
    _sem_branch.END_LAYER = -1
if not hasattr(_sem_branch, "GAMMA_MIN"):
    _sem_branch.GAMMA_MIN = 0.05
if not hasattr(_sem_branch, "GAMMA_MAX"):
    _sem_branch.GAMMA_MAX = 1.0
...
_prompt_sem_branch = _ensure_node(_C.MODEL.PROMPT, "SEMANTIC_BRANCH")
if not hasattr(_prompt_sem_branch, "ENABLE"):
...
if not hasattr(_prompt_sem_branch, "END_LAYER"):
    _prompt_sem_branch.END_LAYER = int(_sem_branch.END_LAYER)
if not hasattr(_prompt_sem_branch, "GAMMA_MIN"):
    _prompt_sem_branch.GAMMA_MIN = float(_sem_branch.GAMMA_MIN)
if not hasattr(_prompt_sem_branch, "GAMMA_MAX"):
    _prompt_sem_branch.GAMMA_MAX = float(_sem_branch.GAMMA_MAX)

_aff = _ensure_node(_C.MODEL, "AFFINITY")
if not hasattr(_aff, "ENABLE"):
    _aff.ENABLE = True
if not hasattr(_aff, "PROMPT_LENGTH"):
    _aff.PROMPT_LENGTH = 0
if not hasattr(_aff, "RETURN_CROSS"):
    _aff.RETURN_CROSS = False
if not hasattr(_aff, "NORMALIZE"):
    _aff.NORMALIZE = True
if not hasattr(_aff, "DETACH"):
    _aff.DETACH = True
if not hasattr(_aff, "VIS"):
    _aff.VIS = False
